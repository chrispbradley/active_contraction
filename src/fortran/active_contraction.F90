...
#include "mpif.h"
  ...
  REAL(CMISSRP), PARAMETER :: START_TIME = 0.0, END_TIME = 10.0, DT = 1  ! ms
...
!  CALL cmfe_Field_ComponentValuesInitialise(MaterialField,CMFE_FIELD_V_VARIABLE_TYPE,CMFE_FIELD_VALUES_SET_TYPE,1,2.0_CMISSRP,Err) ! activate at time 2. TODO: inhomogeneous
  ...
  CALL cmfe_Solver_NewtonJacobianCalculationTypeSet(Solver,CMFE_SOLVER_NEWTON_JACOBIAN_FD_CALCULATED,Err) ! faster than CMFE_SOLVER_NEWTON_JACOBIAN_FD_CALCULATED ?
  ...
  CALL cmfe_Fields_NodesExport(Fields,"./results/ActiveContraction","FORTRAN",Err)
